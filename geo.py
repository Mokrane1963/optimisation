# -*- coding: utf-8 -*-
"""
Created on Tue Oct 14 12:27:48 2025
...
with st.sidebar:
    ...
    # ✅ clé dynamique pour forcer la mise à jour
    liste_pays = sorted(contour_gdf["ADMIN"].unique())
    nom_pays = st.selectbox("Sélectionnez un pays", liste_pays, key=f"pays_select_{st.session_state.get('refresh', 0)}")

# --- Filtrage du pays sélectionné ---
pays_gdf, villes_pays_gdf, capitale = filtrer_pays(contour_gdf, villes_gdf, nom_pays)

# --- Sélecteur + Recherche de ville ---
ville_souhaitee = None
if not villes_pays_gdf.empty:
    liste_villes = sorted(villes_pays_gdf["NAME"].unique())

    # ✅ Clés dynamiques = dépendantes du pays
    ville_souhaitee = st.selectbox("Sélectionnez une ville", liste_villes, key=f"ville_select_{nom_pays}")
    recherche_ville = st.text_input("🔎 Recherchez une ville", key=f"search_{nom_pays}")

    # Recherche directe (insensible à la casse)
    if recherche_ville.strip():
        recherche_ville = recherche_ville.strip().lower()
        match = villes_pays_gdf[villes_pays_gdf["NAME"].str.lower() == recherche_ville]
        if not match.empty:
            ville_souhaitee = match["NAME"].values[0]
            st.success(f"✅ Ville trouvée : {ville_souhaitee}")
        else:
            st.warning(f"Aucune ville nommée '{recherche_ville}' trouvée dans {nom_pays}.")
else:
